...
export function MakePayment({ onBack, profile, applyPayment, linkedAccount, onAddAccount }) {
  ...
  const [payAmount, setPayAmount] = useState(
    profile.statementBalance > 0 ? profile.statementBalance : profile.accountBalance
  );
  ...
  const amount = Math.min(Math.max(0, +payAmount || 0), maxAmount);
  ...
  const SNAP_THRESHOLD = maxAmount * 0.02; // 2% of max for snap range
  ...
  const usagePct = Math.round((profile.accountBalance / profile.creditLimit) * 100);

  function handleCustomChange(e) {
    const raw = e.target.value.replace(/[^0-9.]/g, '');
    const val = Math.min(parseFloat(raw) || 0, maxAmount);
    setPayAmount(val);
    const s = getSnapLabel(val);
    setSelected(s ? s.key : 'custom');
  }
          ...
          <div style={{ position: 'relative', height: 28, marginTop: 8 }}>
            {snapPoints.map((sp) => {
              ...
              return (
                <div
                  key={sp.key}
                  onClick={() => { setPayAmount(sp.value); setSelected(sp.key); }}
                  style={{
                    position: 'absolute',
                    left: `${pct}%`,
