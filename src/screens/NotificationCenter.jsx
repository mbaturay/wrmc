import { CreditCard, Tag, User, Bell, X, ICON_WEIGHT } from '../icons';
...
export function NotificationCenter({
  ...
  if (isNewUser && !biometricEnabled && !nudgeFaceIdDismissed) {
    notifications.push({
      ...
      iconBg: '#F3E8FF',
      iconColor: '#6B21A8',
      icon: <User size={20} weight={ICON_WEIGHT} color="#6B21A8" />,
      ...
      {notifications.map((n) => (
        <div key={n.id} style={{
          ...
          {n.dismissable && (
            <button
              onClick={n.onDismiss}
              style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#CCC', padding: 0, lineHeight: 1, flexShrink: 0 }}
            >
